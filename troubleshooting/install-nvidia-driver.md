...
1. Append `nomodeset ip=dhcp inst.nokill inst.vnc` to the kernel command line. Remove `rhgb` and `quiet` to see the kernel messages scroll by, which may help in further diagnostics.
   * If DHCP is not available on the installation network, the syntax of the `ip=` kernel parameter becomes a bit more involved. The full list of variants is documented in the [Dracut Command-line parameters] (http://man7.org/linux/man-pages/man7/dracut.cmdline.7.html)
...
4. Follow the installation UI. 
   ...
   * Do *not* reboot at the end of the installation. Finish step 5 below first.
5. Once the installation completes, use the local VGA console switch to TTY2 via ALT-F2
   ...
   * Set the root password (this will also enable the root account login) via the `passwd` command
   * Double-check that `/boot/grub2/grub.cfg` contains a `nomodeset` kernel parameter.
     ** If necessary, use `vi /etc/defaults/grub` to modify the grub stub file, then `grub2-mkconfig -o /boot/grub2/grub.cfg` to regenerate the GRUB2 config file.
...
1. Reboot and interrupt the Grub2's process, modifying the kernel parameters to no longer contain `nomodeset`. Use "CTRL-X" to start the boot process.
   * If the LUKS passphrase was set, once the kernel loads, it will be necessary to blindly enter it.
